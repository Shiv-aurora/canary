...
function renderGraph() {
  const c=data.components.find(x=>x.id===selected); const nodes=[["product",20,50,data.product.name],["assembly",48,30,c.assembly],[`component ${c.severity}`,70,30,c.name],["supplier",88,17,"Northstar Components"],["supplier",88,43,"Vector Optics"],["assembly",48,70,"Compute + Power"],["component",70,70,"4 other components"]];
  $("#bom-graph").innerHTML=edge(20,50,48,30)+edge(48,30,70,30)+edge(70,30,88,17)+edge(70,30,88,43)+edge(20,50,48,70)+edge(48,70,70,70)+nodes.map(n=>`<button class="node ${n[0]}" style="left:${n[1]}%;top:${n[2]}%">${n[3]}</button>`).join("");
  const alts=data.alternatives.filter(a=>a.componentId===selected); $("#graph-inspector").innerHTML=`<p class="eyebrow">SELECTED EXPOSURE</p><h3>${c.name}</h3><p>A shortage blocks <b>${data.product.name}</b> through <b>${c.assembly}</b>. There is no approved drop-in replacement.</p><p class="eyebrow">POSSIBLE ALTERNATIVES</p><ul>${alts.length?alts.map(a=>`<li>${a.mpn} · ${Math.round(a.confidence*100)}% evidence confidence</li>`).join(""):"<li>No evidence-backed candidates</li>"}</ul>`;
}
function renderSources(){ $("#source-grid").innerHTML=data.sources.map(s=>`<article class="panel source-card"><header><div><h3>${s.name}</h3><code>${s.collectorId}</code></div><span class="state ${s.state}">● ${s.state}</span></header><div class="source-stats"><span>Freshness<b>${s.freshness}</b></span><span>Rows<b>${s.rows}</b></span><span>Evidence<b>${s.kind}</b></span></div></article>`).join(""); }
function renderTimeline(){ const source=data.sources.find(s=>s.id==="src-controlled"); $("#continuity-id").textContent=source.collectorId; $("#timeline").innerHTML=data.healingEvents.map(e=>`<article class="event ${e.state}"><h3>${e.title}</h3><p>${e.detail}</p><time>${new Date(e.at).toLocaleString()}</time></article>`).join(""); }
