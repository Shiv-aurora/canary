...
test("unknown stock remains valid and is not converted to zero", () => { const errors=validateObservation({componentId:"c",supplierId:"s",sourceId:"x",collectedAt:new Date().toISOString(),inventory:null,provenance:{url:"https://example.com",runId:"r",kind:"live"}}); assert.deepEqual(errors,[]); assert.ok(validateObservation({inventory:-1,provenance:{}}).length>0); });
