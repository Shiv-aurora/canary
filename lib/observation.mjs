...
const currencyFrom = (raw, fallback) => {
  const explicit = text(first(raw, ["currency", "price_currency", "currency_code"]))?.toUpperCase();
...
export function normalizeObservation(raw, collector, { runId = randomUUID(), collectedAt = new Date().toISOString() } = {}) {
  ...
  const availability = normalizeAvailability(first(raw, ["availability", "stock_status", "status", "availability_text"]));
  const currency = currencyFrom(raw, collector.region?.currency);
  const amount = numberFrom(priceRaw);
  const sourceUrl = text(first(raw, ["url", "source_url", "product_url"])) || collector.inputs?.[0]?.url;
  const rawEvidence = text(first(raw, ["rawEvidence", "raw_evidence", "evidence", "availability_text", "stock_status"]));
  const observation = {
    ...
    title: text(first(raw, ["title", "product_name", "name"])),
