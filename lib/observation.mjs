...
export function normalizeObservation(raw, collector, { runId = randomUUID(), collectedAt = new Date().toISOString() } = {}) {
  ...
  const inputUrl = text(raw?.input?.url);
  const inputMetadata = collector.inputs?.find(item => item.url === sourceUrl || item.url === inputUrl);
  ...
  const observation = {
    ...
    componentId: inputMetadata?.componentId || collector.componentId,
    ...
    manufacturer: text(first(raw, ["manufacturer", "brand"])) || inputMetadata?.manufacturer || collector.manufacturer || null,
