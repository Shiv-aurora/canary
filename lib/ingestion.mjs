...
export class IngestionService {
  ...
  async collect(keyOrId, { inputs, trigger = "manual" } = {}) {
    ...
    try {
      const collectorInputs = inputs || collector.inputs || [];
      const triggerInputs = collectorInputs.map(item => item?.url ? { url: item.url } : item);
      const queued = await this.client.triggerCollector(collector.collectorId, triggerInputs);
      ...
      const inputCount = collectorInputs.length;
      const rows = await this.client.pollDataset(run.snapshotId, { timeoutMs: Math.min(240_000, 60_000 + inputCount * 15_000) });
