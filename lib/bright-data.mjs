...
export class BrightDataClient {
  constructor({ token = process.env.BRIGHT_DATA_API_TOKEN, baseUrl = process.env.BRIGHT_DATA_API_BASE || "https://api.brightdata.com" } = {}) {
    this.token = token;
  ...
  async request(path, options = {}) {
    ...
    if (!response.ok) throw new Error(`Bright Data ${response.status}: ${await response.text()}`);
